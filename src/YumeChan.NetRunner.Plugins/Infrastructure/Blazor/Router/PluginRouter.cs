...
/// </summary>
public class PluginRouter : IComponent, IHandleAfterRender, IDisposable
{
    ...
    private ILogger<PluginRouter> _logger;
    ...
    private Regex _pluginFullPathRegex = new(@"^(?:/?)(?:p|plugin)/([\w._]+)(/.*)", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    
    /// <inheritdoc />
    public void Attach(RenderHandle renderHandle)
    {
        _logger = LoggerFactory.CreateLogger<PluginRouter>();
    ...
    private void OnLocationChanged(object sender, LocationChangedEventArgs args)
    {
        ...
        string path = NavigationManager.ToBaseRelativePath(args.Location);
        
        // Check if plugin full path is used, if so, strip off the plugin base path
        if (_pluginFullPathRegex.Matches(path) is { Count: > 0 } pathMatches)
        {
            path = pathMatches.FirstOrDefault()?.Groups[2].Value ?? "/";
        }

        if (_renderHandle.IsInitialized)
        {
            _ = RunOnNavigateAsync(path, args.IsNavigationIntercepted).Preserve();
        }
    }
    ...
    private static class Log
    {
        private static readonly Action<ILogger, string, string, Exception> _displayingNotFound =
            LoggerMessage.Define<string, string>(LogLevel.Debug, new(1, "DisplayingNotFound"), 
                $"[PluginRouter] Displaying {nameof(NotFound)} because path '{{Path}}' with base URI '{{BaseUri}}' does not match any component route");

        private static readonly Action<ILogger, Type, string, string, Exception> _navigatingToComponent =
            LoggerMessage.Define<Type, string, string>(LogLevel.Debug, new(2, "NavigatingToComponent"), 
                "[PluginRouter] Navigating to component {ComponentType} in response to path '{Path}' with base URI '{BaseUri}'");
        ...
        private static readonly Action<ILogger, string, string, string, Exception> _navigatingToExternalUri =
            LoggerMessage.Define<string, string, string>(LogLevel.Debug, new(3, "NavigatingToExternalUri"), 
                "[PluginRouter] Navigating to non-component URI '{ExternalUri}' in response to path '{Path}' with base URI '{BaseUri}'");
        ...
        internal static void NavigatingToComponent(ILogger logger, Type componentType, string path, string baseUri) => _navigatingToComponent(logger, componentType, path, baseUri, null);
